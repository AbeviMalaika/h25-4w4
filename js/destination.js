...
(function () {
    ...
    (function parcourir_bouton() {
        ...
        categorie__ul__li.forEach(elm => {
            elm.addEventListener("click", afficherPaysDestination);
        })
    })()
    ...
    async function afficherPaysDestination(event) {
        const dataDestinationID = event.target.getAttribute('data-destinationID'); // Remplacez par l'ID de la catégorie souhaitée
        const apiUrl = `${domaine}wp-json/wp/v2/posts?search=${dataDestinationID}`; //Mettre méthode
        // const apiUrl = `${domaine}wp-json/wp/v2/search/?search=france`;
        console.log(apiUrl);
        fetch(apiUrl)
            .then(response => response.json())
            .then(data => {
